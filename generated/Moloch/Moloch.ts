...
export class SubmitProposal__Params {
  ...
  get flags(): Array<boolean> {
    return this._event.parameters[10].value.toBooleanArray();
  }
}
...
export class Moloch__proposalsResult {
  ...
  toMap(): TypedMap<string, EthereumValue> {
    ...
    map.set("value12", EthereumValue.fromString(this.value12));
...
export class Moloch extends SmartContract {
  ...
  summoner(): Address {
    let result = super.call("summoner", []);

    return result[0].toAddress();
  }

  try_summoner(): CallResult<Address> {
    let result = super.tryCall("summoner", []);
    if (result.reverted) {
      return new CallResult();
    }
    let value = result.value;
